...
function _send_api($from, $from_name, $to, $to_name, $subject, $message, $plain_text_message, $cc_emails=null, $bcc_emails=null, $isTransactional=TRUE, $msgTo) {
	...
	try {
		...
		$post = array(
			'from'            => $from,
			'fromName'        => $from_name,
			'apikey'          => 'FA90B965751BBBCC6E41F3F82BBDA05AD2DE160007CAACD5D4EE2326792015E051DBD3008D3813C135D66D6B16020C03',
			'subject'         => $subject,
			'to'              => implode(',', $to),
			'msgTo'           => $msgTo,
		...
		$result = curl_exec($ch);
		curl_close($ch);
		//error_log("Result Curl:".$result);
		//error_log("Json API Data:".json_encode($post));
		return $result;
	}
	catch(Exception $ex){
